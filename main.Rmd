...
A possible solution: The "SWAG" algorithm proposed in Guerrier et al. (2016) is an heuristic approach that aims to address (some of ) these issues.  Its goals are the following: Finding ALL models (and parameters) minimizing an out-of-sample objective function (e.g.  classification error).Restricting our attention to the models with the smallest dimension.
...
Most statistical methods are used to deliver estimates and predictions for different problems with the goal of using these results for decision-making(e.g.  whether a patient should be treated, selecting a drug or its dosage).The vast majority of these methods are independent of the context of this decision-making process and the approach used to obtain model’s parameters (and prediction) is fixed for all applications.This can lead to contradictory examples(e.g.  asymmetric “risk” of treating vs not treating a patient).Indeed, not all prediction errors are the “same” or have similar “impacts”.
...
We first consider a simple variable selection method based on the Akaike information criterion  (AIC). We consider a stepwise AIC-based variable selection method. This is a standard approach and proceed to greedy variable selection based on the AIC. Considering this method, we obtain the following estimated model:

---

# .smallest[Customer churn: Dataset]

We consider the [*Telco customer churn*](https://community.ibm.com/community/user/businessanalytics/blogs/steven-macko/2019/07/11/telco-customer-churn-1113) that provides information about customers of a telecom company and whether or not left the clientele of the company.
...
---
# .smallest[Customer churn: Simple analysis]
...
We find many models with equivalent estimated out-of-sample prediction error.
...
We can represent the relationship between variables in the best performing models (with respect to the estimated out of sample error) with such a graph. Each chord represent the number of time two variables were included in a model among the best performing models.
